...
public class TE_Behavior_Energy_Stats extends TE_Behavior_Energy {
	...
	public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H, String aSides, boolean aEmitting) {
		aList.add((aEmitting ? (Chat.RED + LH.get(LH.ENERGY_OUTPUT)) : (Chat.GREEN + LH.get(LH.ENERGY_INPUT))) + ": " + Chat.WHITE + mRec + " " + mType.getLocalisedChatNameShort() + Chat.WHITE + "/t ("+(mMin>1?LH_CH.getNumber(LH_CH.ENERGY_TO, mMin, mMax):LH_CH.getNumber(LH_CH.ENERGY_UPTO, mMax))+(UT.Code.stringInvalid(aSides)?"":", "+aSides)+")");
